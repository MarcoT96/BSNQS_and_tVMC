...
void PrintInfo(){  //Print informations for command line instructions

  std::cout << "\nTo execute the main code: ./main.exe OPTIONS" << std::endl;
  std::cout << "  ================" << std::endl;
...
std::map <std::string, std::string> setOptions(int argc, char* argv[]) {  //Prepare all the stuff for the simulation
    ...
    switch(c){
      ...
      case 'a':
        ...
        if(std::string(optarg) != "None")
          std::cout << " Initial wave function variational parameters uploaded from file " << std::string(optarg) << std::endl;
        break;

      case 'b':
        options["model"] = optarg;
        std::cout << " Quantum model to be simulated: " << std::string(optarg) << std::endl;
        break;

      case 'c':
        options["ansatz"] = optarg;
        std::cout << " Type of variational wave function ansatz: " << std::string(optarg) << std::endl;
        break;

      case 'd':
        options["input"] = optarg;
        std::cout << " Read simulation parameters from the file " << std::string(optarg) << std::endl;
        break;

      case 'e':
        options["integrator"] = optarg;
        std::cout << " ODE integrator for the TDVMC equations of motion: " << std::string(optarg) << std::endl;
        break;
  ...
  if(options.count("model")==0)
    options["model"] = "Ising1d";
  if(options.count("ansatz")==0)
    options["ansatz"] = "JasNN";
