...
int main(int argc, char* argv[]){
  ...
  if(options["ansatz"] == "JasNN"){  //Nearest-neighbors Jastrow Quantum State
    ...
    if(options["filename"] != "None")
      Psi = new JasNN(options["filename"], std::stoi(options["if_phi_neq_zero"]), rank);
    else
      Psi = new JasNN(std::stoi(options["n_visible"]), std::stoi(options["if_phi_neq_zero"]), rank);

  }
  else if(options["ansatz"] == "RBM"){  //Neural Network Quantum State

    if(options["filename"] != "None")
      Psi = new RBM(options["filename"], std::stoi(options["if_phi_neq_zero"]), rank);
    else
      Psi = new RBM(std::stoi(options["n_visible"]), std::stoi(options["density"]), std::stoi(options["if_phi_neq_zero"]), rank);

  }
  else if(options["ansatz"] == "uRBM"){  //Shadow (quasi)-uRBM Quantum State

    if(options["filename"] != "None")
      Psi = new quasi_uRBM(options["filename"], std::stoi(options["if_phi_neq_zero"]), rank);
    else
      Psi = new quasi_uRBM(std::stoi(options["n_visible"]), std::stoi(options["if_phi_neq_zero"]), rank);

  }
  else if(options["ansatz"] == "BS_NNQS"){  //Baeriswyl-Shadow Quantum State

    if(options["filename"] != "None")
      Psi = new BS_NNQS(options["filename"], std::stoi(options["if_phi_neq_zero"]), rank);
    else
      Psi = new BS_NNQS(std::stoi(options["n_visible"]), std::stoi(options["if_phi_neq_zero"]), rank);
  ...
  if(std::stoi(options["if_write_final_config"])) sampler.setFile_final_Config(options["file_info"], rank);
  ...
  if(std::stoi(options["if_hidden_off"])) sampler.ShutDownHidden(std::stoi(options["orientation_bias"]));
  ...
  if(std::stoi(options["if_vmc"]) == false && rank == 0) std::cout << " Quantum State evolved." << std::endl;
