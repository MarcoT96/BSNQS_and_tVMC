...
class VMC_Sampler {
  ...
  private:
    ...
    Col <std::complex <double>> _mean_O;  // ⟨⟨𝕆ₖ⟩ᵇˡᵏ⟩
    Col <std::complex <double>> _mean_O_star;  // ⟨⟨𝕆⋆ₖ⟩ᵇˡᵏ⟩
  ...
  public:
    ...
    Col <double> _O_angled() const {return _mean_O_angled;}  //Returns the Monte Carlo estimate of the vector of ≪𝕆ₖ≫
    Col <double> _O_square() const {return _mean_O_square;}  //Returns the Monte Carlo estimate of the vector of ⌈𝕆ₖ⌋
...
void VMC_Sampler :: setStepParameters(unsigned int Nsweeps, unsigned int Nblks, unsigned int Neq, unsigned int M,
  ...
  if(rank == 0){
    ...
    std::cout << " \tNumber of spin-flips moves in the single 𝑴𝑪 sweep = " << _M << std::endl;
...
void VMC_Sampler :: setFile_final_Config(std::string info, int rank) {
  ...
  if(!_file_final_Config.good()){
    ...
    std::cerr << " ##FileError: Cannot open the file ‹‹ final_config_" << info << "node_" << rank << ".dat ›› for writing the final configurations at the end of each 𝐕𝐌𝐂 step." << std::endl;
    std::abort();
    ...
    if(rank == 0)
      std::cout << " Saving the final configurations sampled at the end of each 𝐕𝐌𝐂 step on file ‹‹ final_config_" << info << "_node_*.dat ››." << std::endl;
...
void VMC_Sampler :: Write_Quantum_properties(unsigned int tdvmc_step, int rank) {
  ...
  if(rank == 0){
    ...
    if(!_if_shadow){
      ...
      Col <double> shadow_energy = real(_global_Observables(0, 0)) / _global_cosII;  //Computes ⟨ℋ⟩ⱼᵇˡᵏ/⟨𝑐𝑜𝑠⟩ⱼᵇˡᵏ in each block
      Col <double> shadow_Sx = real(_global_Observables(1, 0)) / _global_cosII;  //Computes ⟨σ̂ˣ⟩ⱼᵇˡᵏ/⟨𝑐𝑜𝑠⟩ⱼᵇˡᵏ in each block
      Col <double> shadow_Sy = real(_global_Observables(2, 0)) / _global_cosII;  //Computes ⟨σ̂ʸ⟩ⱼᵇˡᵏ/⟨𝑐𝑜𝑠⟩ⱼᵇˡᵏ in each block
      Col <double> shadow_Sz = real(_global_Observables(3, 0)) / _global_cosII;  //Computes ⟨σ̂ᶻ⟩ⱼᵇˡᵏ/⟨𝑐𝑜𝑠⟩ⱼᵇˡᵏ in each block
...
void VMC_Sampler :: compute_QGTandGrad(MPI_Comm common) {
  ...
  if(!_if_shadow){
    ...
    for(unsigned int m = 0; m < n_alpha; m++){
      ...
      for(unsigned int n = 0; n < n_alpha; n++){
        ...
        for(unsigned int block_ID = 0; block_ID < _Nblks; block_ID++){
          ...
          Q(m, n) += block_qgt / double(blk_size);  // ⟨𝙎ₘₙ⟩ᵇˡᵏ
    ...
    Col <double> mean_O_angled(n_alpha);  // ⟨≪𝕆ₖ≫ᵇˡᵏ⟩ with reweighting correction
    Col <double> mean_O_square(n_alpha);  // ⟨⌈𝕆ₖ⌋ᵇˡᵏ⟩ with reweighting correction
    ...
    E.real(mean(real(_Observables(0, 0))) / mean_cos);  // ⟨⟨ℋ⟩ᵇˡᵏ⟩ with reweighting correction
    E.imag(0.0);
  ...
  if(!_if_vmc){
    ...
    if(!_if_shadow){
      ...
      if(rank == 0){
        ...
        new_alpha = alpha_t + 0.5 * _delta * (alpha_dot_t + alpha_dot_tilde_t);  // 𝜶(𝑡 + 𝛿𝑡)
  ...
  if(!_if_vmc){
    ...
    if(!_if_shadow){
      ...
      if(rank == 0){
        ...
        //Updates the variational parameters
        new_alpha = alpha_t + 0.5 * _delta * k1;  // 𝛼(𝑡) + 𝟣/𝟤•𝛿ₜ•κ𝟣
      ...
      if(rank == 0){
        ...
        //Updates the variational parameters
        new_alpha = alpha_t + 0.5 * _delta * k2;  // 𝛼(𝑡) + 𝟣/𝟤•𝛿ₜ•κ𝟤
      ...
      if(rank == 0){
        ...
        //Updates the variational parameters
        new_alpha = alpha_t + _delta * k3;  // 𝛼(𝑡) + 𝛿ₜ•κ𝟥
      ...
      if(rank == 0){
        ...
        //Final update of the variational parameters
        new_alpha = alpha_t + (1.0/6.0) * _delta * (k1 + k2 + k3 + k4);  // 𝛼ₖ(𝑡 + 𝛿ₜ)
